...
export default function ColetaDadosPage() {
  ...
  // Estado para montar o Portal na Header
  const [headerSlot, setHeaderSlot] = useState(null);
  useEffect(() => {
    setHeaderSlot(document.getElementById('header-coleta-buttons-slot'));
  }, []);
  ...
  // Botões que serão injetados na Header através do Portal
  const actionButtons = (
    <>
      <button 
        onClick={() => {
      ...
      >
        {lang === 'en-US' ? '+ Initial Collection' : '+ Cadastrar Coleta'}
      </button>
      ...
      >
        {lang === 'en-US' ? '+ Quantification' : '+ Nova Setorização'}
      </button>
    </>
  );
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1200px', fontFamily: 'sans-serif' }}>
      ...
      {/* INSERE OS BOTÕES NA HEADER APENAS QUANDO ESTIVER NESSA PÁGINA */}
