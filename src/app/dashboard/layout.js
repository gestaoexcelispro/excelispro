...
export default function DashboardLayout({ children }) {
  ...
  const getBreadcrumb = () => {
    ...
    if (pathname.includes('/diretoria/mapa')) {
      return `${t.directorateModule} > ${t.mapDashboard}`;
    }

    if (pathname.includes('/diretoria/planejamento')) {
      return `${t.directorateModule} > ${t.strategicPlan}`;
    }

    if (pathname.includes('/projetos/lista')) {
      return `${t.projectsModule} > ${t.projectList}`;
    }

    if (pathname.includes('/projetos/coleta')) {
      return `${t.projectsModule} > ${t.coletaDados}`;
    }

    if (pathname.includes('/projetos/masterplan')) {
      return `${t.projectsModule} > ${t.masterPlan}`;
    }

    if (pathname.includes('/projetos/lookahead')) {
      return `${t.projectsModule} > ${t.lookahead}`;
    }

    if (pathname.includes('/projetos/semanal')) {
      return `${t.projectsModule} > ${t.weeklyPlan}`;
    }

    if (pathname.includes('/projetos/matriz-status')) {
      return `${t.projectsModule} > ${t.statusMatrix}`;
    }

    if (pathname.includes('/projetos/tap')) {
      return `${t.projectsModule} > ${t.tap}`;
    }

    if (pathname.includes('/projetos/plano')) {
      return `${t.projectsModule} > ${t.managementPlan}`;
    }

    if (pathname.includes('/projetos/raci')) {
      return `${t.projectsModule} > ${t.raci}`;
    }

    if (pathname.includes('/projetos/riscos')) {
      return `${t.projectsModule} > ${t.risks}`;
    }

    if (pathname.includes('/orcamentos')) {
      return `${t.commercialModule} > ${t.budgets}`;
    }

    if (pathname.includes('/propostas')) {
      return `${t.commercialModule} > ${t.proposals}`;
    }

    if (pathname.includes('/modelos')) {
      return `${t.commercialModule} > ${t.templates}`;
    }

    if (pathname.includes('/servicos')) {
      return `${t.commercialModule} > ${t.services}`;
    }

    if (pathname.includes('/controladoria/receber')) {
      return `${t.financialModule} > ${t.receivable}`;
    }

    if (pathname.includes('/controladoria/pagar')) {
      return `${t.financialModule} > ${t.payable}`;
    }

    if (pathname.includes('/juridico/contratos')) {
      return `${t.legalModule} > ${t.contracts}`;
    }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div>
              ...
              <span style={{ fontSize: '0.65rem', color: '#90cdf4' }}>
                {t.subtitle}
              </span>
            </div>
          </div>
          ...
          {isColetaPage && (
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              >
                {lang === 'en-US'
                  ? '+ Quantification'
                  : '+ Nova Quantificação e Classificação'}
              </button>
            </div>
          )}
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        >
          <div style={{ width: '260px' }}>
            ...
            >
              ...
              <button
                onClick={() =>
                  setIsDirectorateOpen(!isDirectorateOpen)
                }
                style={{
                  background: 'none',
                  border: 'none',
            ...
            {isDirectorateOpen && (
              ...
              >
                ...
                >
                  <span>🗺️</span> {t.mapDashboard}
                </a>

                <a
                  href="/dashboard/diretoria/planejamento"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '10px',
            ...
            {isProjectsOpen && (
              ...
              >
                ...
                >
                  <span>🚥</span> {t.statusMatrix}
                </a>

                <a
                  href="/dashboard/projetos/tap"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '10px',
                ...
                >
                  <span>📑</span> {t.managementPlan}
                </a>

                <a
                  href="/dashboard/projetos/raci"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '10px',
                ...
                <a
                  href="/dashboard/projetos/riscos"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '10px',
            ...
            {isFinancialOpen && (
              ...
              >
                <a
                  href="/dashboard/controladoria/receber"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '10px',
                ...
                <a
                  href="/dashboard/controladoria/pagar"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '10px',
        ...
        >
          {cloneElement(children, {
            registerModalTriggers: (triggers) =>
              setModalTriggers(triggers)
          })}
        </main>
      </div>
    </div>
